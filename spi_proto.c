...
//TODO replace all 16s with 15s so that 0b1111 can be used as a dummy SEQ number

//update the protocol state with a message. Doesn't do any processing of the message itselves
void
spi_proto_rcv_msg(struct spi_state *s, struct spi_packet *p)
{
...
spi_msg_crc(struct spi_packet *p)
{
	...
	uint16_t crc_res = crc16_block(0, (void *) p, sizeof (struct spi_packet) - 2);
	//p->crc = crc_res;
	return crc_res;
}